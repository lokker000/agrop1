...
//
// GET /api/pronostico                          -> 14 días, Los Lingues (por defecto)
// GET /api/pronostico?dias=7                    -> 7 días
// GET /api/pronostico?stationId=INIA-202&dias=7 -> estación elegida (cualquier región)
import { NextResponse } from "next/server";
import { fetchForecast, resolveStationLocation, LOS_LINGUES } from "@/lib/forecast";
...
export async function GET(request: Request) {
  ...
  try {
    const loc = stationId
      ? resolveStationLocation(stationId)
      : LOS_LINGUES;
    const data = await fetchForecast(loc, dias);
