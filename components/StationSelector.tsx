...
import { CloudIcon, CheckIcon, AlertIcon } from "./icons";

interface Props {
  onData: (data: HourlyRecord[]) => void;
  from: string; // YYYY-MM-DD (inicio dormancia)
  to: string; // YYYY-MM-DD (fecha final; si vacío se usa hoy)
}
...
export default function StationSelector({ onData, from, to }: Props) {
  ...
  async function load() {
    ...
    setLoaded(0);
    if (!from) {
      setError("Selecciona primero la fecha de inicio de dormancia.");
      return;
    }
    setLoading(true);
    try {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl bg-sky-50 p-4 ring-1 ring-sky-100">
        ...
        <p className="flex-1 text-xs text-sky-800">
          ...
          Agrometeorológica de INIA (agrometeorologia.cl). Rango:{" "}
